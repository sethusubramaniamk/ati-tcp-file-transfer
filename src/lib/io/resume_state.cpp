...
ResumeState::ResumeState(ManifestId id, uint32_t chunk_count)
    : manifest_id_(id), chunk_count_(chunk_count),
...
bool ResumeState::load(const std::filesystem::path& path, ResumeState& out) {
    ...
    char magic[4]{};
    f.read(magic, 4);
    if (f.gcount() != 4 || std::memcmp(magic, kMagic.data(), 4) != 0) return false;

    char ver = 0;
    f.read(&ver, 1);
    if (f.gcount() != 1 || static_cast<uint8_t>(ver) != kVersion) return false;

    ManifestId id{};
    f.read(reinterpret_cast<char*>(id.data()), static_cast<std::streamsize>(id.size()));
    if (static_cast<size_t>(f.gcount()) != id.size()) return false;

    char cc_bytes[4]{};
    f.read(cc_bytes, 4);
    if (f.gcount() != 4) return false;
    const uint32_t cc =
        static_cast<uint32_t>(static_cast<unsigned char>(cc_bytes[0])) |
    ...
    if (bitmap_bytes > 0) {
        ...
        if (static_cast<size_t>(f.gcount()) != bitmap_bytes) return false;
    }

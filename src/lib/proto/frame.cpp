...
std::optional<FrameHeader> FrameHeader::decode_from(std::span<const std::byte, kHeaderSize> src,
                                                    size_t max_payload,
    ...
    if (!is_known_frame_type(type_byte)) {
        if (err_out != nullptr)
            *err_out = DecodeError::UnknownType;
        return std::nullopt;
    }
    ...
    if (static_cast<size_t>(len) > max_payload) {
        if (err_out != nullptr)
            *err_out = DecodeError::PayloadTooLarge;
        return std::nullopt;
    }
...
std::vector<std::byte> encode_frame(FrameType type, std::span<const std::byte> payload) {
    ...
    if (!payload.empty()) {
        std::copy(
            payload.begin(), payload.end(), buf.begin() + static_cast<std::ptrdiff_t>(kHeaderSize));
    }
    return buf;
}

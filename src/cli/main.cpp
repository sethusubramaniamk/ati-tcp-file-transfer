...
[[nodiscard]] bool parse_host_port(const std::string& s, std::string& host, uint16_t& port) {
...
[[nodiscard]] std::optional<ftx::transport::TlsConfig> build_tls_config(const CommonTlsArgs& a,
                                                                        bool is_server) {
    if (a.insecure)
        return std::nullopt;
    if (a.cert.empty() || a.key.empty()) {
        spdlog::error(
            "TLS is required by default; pass --insecure or provide --tls-cert + --tls-key");
        return std::nullopt;
    }
...
void add_tls_options(CLI::App* sub, CommonTlsArgs& a) {
    ...
    sub->add_flag(
        "--no-verify-peer", a.no_verify_peer, "do not require / verify a peer certificate");
    sub->add_option("--sni", a.sni, "client only — server name to send + verify against");
    sub->add_flag(
        "--insecure", a.insecure, "disable TLS entirely (plain TCP — local testing only)");
}
...
int run_send(const SendOpts& o) {
    ...
    const std::string remote_dest = o.out.empty() ? src.filename().generic_string() : o.out;
...
int main(int argc, char** argv) {
    ...
    serve
        ->add_option(
            "--listen", serve_opts.listen_addr, "host:port to listen on (default 0.0.0.0:9000)")
        ->capture_default_str();
    serve->add_option("--root", serve_opts.root, "root directory for incoming files")->required();
    add_tls_options(serve, serve_opts.tls);

    SendOpts send_opts;
    auto* send = app.add_subcommand("send", "Send a file to a remote ftx serve");
    send->add_option("remote", send_opts.remote, "host:port of remote ftx serve")->required();
    send->add_option("source", send_opts.source, "local file to send")->required();
    send->add_option("--out", send_opts.out, "remote destination path (default: source filename)");
    send->add_option("--chunk-size", send_opts.chunk_size, "chunk size in bytes (default 1 MiB)")
        ->capture_default_str();

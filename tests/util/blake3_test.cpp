...
ftx::HashDigest hex_to_digest(std::string_view hex) {
    ...
    for (size_t i = 0; i < out.size(); ++i) {
        ...
        const auto h = static_cast<unsigned char>((hi >= 'a') ? (hi - 'a' + 10) : (hi - '0'));
        const auto l = static_cast<unsigned char>((lo >= 'a') ? (lo - 'a' + 10) : (lo - '0'));
...
TEST(Blake3, EmptyDigest) {
    ...
    const auto expected =
        hex_to_digest("af1349b9f5f9a1a6a0404dea36dcc9499bcb25c9adc112b7cc9a93cae41f3262");
...
TEST(Blake3, IetfShortInput) {
    ...
    const auto expected =
        hex_to_digest("83a2de1ee6f4e6ab686889248f4ec0cf4cc5709446a682ffd1cbb4d6165181e2");
    EXPECT_EQ(got, expected);
}
